...
image raster_scan(image filtered_image){
	...
	for(int y = 0; y < (labeled_image.h); y++){
			for(int x = 0; x < (labeled_image.w); x++){
				// sets current pixel
				current_pixel = labeled_image.data[y + x]; // this might be very wrong; too tired to do math rn
				// found foreground
				if(current_pixel == -1){
					// looks to the needed adjacent pixel to see if they're already labeled
					top_pixel = find_pixel_conv(labeled_image, x - 1, y - 2, 0); // top middle
					left_pixel = find_pixel_conv(labeled_image, x - 2, y - 1, 0); // middle left
					...
					labeled_image.data[y + x] = current_pixel;
				}
			}
	}
